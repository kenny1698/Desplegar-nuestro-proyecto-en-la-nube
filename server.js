...
C:\Program Files\MongoDB\Server\5.0\bin>mongod --dbpath "C:\Users\u585715\Desktop\Curso\miBaseMongo"

npm i -g artillery@1.7.9
node --prof server.js 8080 FORK
artillery quick --count 50 -n 20 http://localhost:8080/info > result_forkBloq.txt
node --prof server.js 8080 FORK
artillery quick --count 50 -n 20 http://localhost:8080/info > result_forkNoBloq.txt
...
const contProd = new Contenedor(optionsSQLite)
...
const logger  = require('./log4js/log4js-module')

dotenv.config()

const secretmongo = process.env.secretMongo
//const mongodb = process.env.mongoDB
const mongodb = "mongodb://kenny1698:kenny1698@cluster0-shard-00-00.g7tte.mongodb.net:27017,cluster0-shard-00-01.g7tte.mongodb.net:27017,cluster0-shard-00-02.g7tte.mongodb.net:27017/myFirstDatabase?ssl=true&replicaSet=atlas-13vjwv-shard-0&authSource=admin&retryWrites=true&w=majority"
...
if (modoCluster && cluster.isMaster) {
  ...
  app.get('/infozip', compression(),(req, res)=>{
    ...
    //console.log('get /infozip')
     // res.send(`Servidor expressen PORT ${PORT} -PID ${process.pid}`)
      const welcome = `Servidor expressen PORT ${PORT} -PID ${process.pid}`
      res.send(welcome.repeat(1000))
  })

  app.get('/api/randomsnoBloq', (req, res) => {
  ...
  app.get('/api/randomsBloq', (req, res) => {
    let cant = req.query.cant
    if (cant == undefined)
      cant = 100000000
    const resultado = computoBloqueante(cant)
    res.json({ resultado})
  })

  function computoBloqueante(cant){
    const cantArray=[]
    const obj =[]
    let count = 1
